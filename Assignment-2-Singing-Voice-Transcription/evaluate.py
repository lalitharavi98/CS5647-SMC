...
def eval_one_data(answer_true, answer_pred, onset_tolerance=0.05, shifting=0, gt_pitch_shift=0):
    ...
    raw_data = transcription.evaluate(ref_intervals, ref_pitches, est_intervals, est_pitches,
                                      onset_tolerance=onset_tolerance, pitch_tolerance=50)
...
def eval_all(answer_true, answer_pred, onset_tolerance=0.05, shifting=0, print_result=True, id_list=None):
    ...
    if print_result:
        ...
        print("Ground truth note num:", avg[9], "Pred note num:", avg[10])
...
class MirEval():
    ...
    def accuracy(self, onset_tolerance, print_result=True):
        return eval_all(self.gt, self.pred, onset_tolerance=onset_tolerance, print_result=print_result,
